// Test patterns for Sha 2 as per FIPS 180-4
...
func TestGolden384512(t *testing.T) {
	for i := 0; i < len(golden384); i++ {
		//fmt.Println("Test Sha384:", i)
		g := golden384[i]
		b := Sha384(bytes.NewBufferString(g.in))
		s := fmt.Sprintf("%x", b.Bytes())
		if s != g.out {
			t.Fatalf("Sum384 function: sha384(%s) = %s want %s", g.in, s, g.out)
		}
	}
	for i := 0; i < len(golden512); i++ {
		//fmt.Println("Test Sha512:", i)
		g := golden512[i]
		b := Sha512(bytes.NewBufferString(g.in))
		s := fmt.Sprintf("%x", b.Bytes())
		if s != g.out {
			t.Fatalf("Sum512 function: sha512(%s) = %s want %s", g.in, s, g.out)
		}
	}
	for i := 0; i < len(golden512_224); i++ {
		//fmt.Println("Test Sha512_224:", i)
		g := golden512_224[i]
		b := Sha512_224(bytes.NewBufferString(g.in))
		s := fmt.Sprintf("%x", b.Bytes())
		if s != g.out {
			t.Fatalf("Sum512_224 function: sha512_224(%s) = %s want %s", g.in, s, g.out)
		}
	}
	for i := 0; i < len(golden512_256); i++ {
		//fmt.Println("Test Sha512_256:", i)
		g := golden512_256[i]
		b := Sha512_256(bytes.NewBufferString(g.in))
		s := fmt.Sprintf("%x", b.Bytes())
		if s != g.out {
			t.Fatalf("Sum512_256 function: sha512_256(%s) = %s want %s", g.in, s, g.out)
		}
	}
}

func benchmarkSizeSha384(b *testing.B, size int) {
	b.SetBytes(int64(size))
	for i := 0; i < b.N; i++ {
		Sha384(bytes.NewBuffer(buf[:size]))
	}
}

func BenchmarkHash8BytesSha384(b *testing.B) {
	benchmarkSizeSha384(b, 8)
}

func BenchmarkHash1KSha384(b *testing.B) {
	benchmarkSizeSha384(b, 1024)
}

func BenchmarkHash8KSha384(b *testing.B) {
	benchmarkSizeSha384(b, 8192)
}

func benchmarkSizeSha512(b *testing.B, size int) {
	b.SetBytes(int64(size))
	for i := 0; i < b.N; i++ {
		Sha512(bytes.NewBuffer(buf[:size]))
	}
}

func BenchmarkHash8BytesSha512(b *testing.B) {
	benchmarkSizeSha512(b, 8)
}

func BenchmarkHash1KSha512(b *testing.B) {
	benchmarkSizeSha512(b, 1024)
}

func BenchmarkHash8KSha512(b *testing.B) {
	benchmarkSizeSha512(b, 8192)
}
